...
struct GenericDate {
  ...
  int year;
  GenericDate() = default;
  GenericDate(short d, short m, int y) : day(d), month(m), year(y) {}
};
 ...
 public:
  ...
  void show() const {
    ...
    if (!is_completed) {
      std::cout << "Task: " << task;
      std::cout << "\nDesc: " << description;
      std::cout << "\nDate: " << created_date.day << "-" << created_date.month
                << "-" << created_date.year << "\n";
    }
...
void LoadFromFile(std::string filename) {
  ...
  for (int i{0}; i < tokens.size() - 4; i += 5) {
    ...
    try {
      ...
      std::cerr << "Invalid argument: " << e.what() << " - Token: ["
                << tokens[i] << "]\n";
    } catch (const std::out_of_range& e) {
      std::cerr << "Out of range error: " << e.what() << " - Token: ["
                << tokens[i] << "]\n";
    }
